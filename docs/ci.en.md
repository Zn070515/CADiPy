...
The workflow rejects a pre-existing `SLDWORKS.exe` so the job never attaches to an uncontrolled existing instance. Preflight creates a new owned hidden instance with `launch(visible=False)`, verifies the revision and visibility, and closes it. The integration fixture closes only CADiPy-created documents and instances. The test lifecycle forbids calling `application.launch` through an attached session because the current backend can reclassify the existing application as launch-owned and may call `ExitApp` during disconnect.
