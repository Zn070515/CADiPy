...
The workflow rejects a pre-existing `SLDWORKS.exe` so the job never attaches to an uncontrolled existing instance. Preflight creates a new owned hidden instance with `launch(visible=False)`, verifies the revision and visibility, and closes it. The integration fixture closes only CADiPy-created documents and instances. The backend enforces an application ownership state machine: same-mode acquisition is idempotent, while a conflicting attach/launch acquisition raises `ApplicationOwnershipError` without reclassifying the existing application; disconnect exits only a launch-owned instance.
...
Strict session tests use the public session façade and assert only serializable results and semantic evidence. They verify that executor creation, connection, operations, and disconnect all run on one STA host thread. The 100×60×3 mm rectangular round trip creates the model, rebuilds, inspects, saves, closes, reopens, and verifies it again. A required verification failure is asserted through the typed exception and direct `OperationResult.failure` semantic path; these strict real-COM tests do not call `ProtocolServer.handle()` or MCP. Portable protocol adapter tests cover the RPC/MCP `ok=false` envelopes. The attached user-owned lifecycle test creates its application after preflight, avoids the attach-to-launch transition, and confirms that it remains available after CADiPy disconnects.
...
Preflight rejects any `SLDWORKS.exe` that existed before the job, so the strict job does not attach to it. The fixture cleans up only instances and documents explicitly created and owned by the job; the backend also rejects an attach-to-launch ownership conflict with `ApplicationOwnershipError`. An ordinary command exception is delivered to its caller while the host continues. On timeout, the caller receives the built-in `TimeoutError`, the host moves to `failed`, and later submissions receive `WorkerError`; worker-loop/startup/cleanup failure also moves the host to `failed` and rejects later work. A COM/process crash has no automatic-recovery guarantee; timeout or uncertain-rollback evidence requires session cleanup and reconnection, not an automatic retry. The execution runtime makes no ACID or exactly-once guarantee.
